...
<section class=" bg-gray-50 overflow-scroll w-fit p-5 m-5 rounded-md ">
    ...
    <div class="  text-end">
        <a href="./batch-create.php" type="button" name="calculate" class=" max-w-sm px-3 py-2 text-sm font-semibold rounded-lg border border-transparent bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-50 disabled:pointer-events-none">
            Create Batch
        </a>
    </div>
    ...
    <div class=" min-w-full overflow-x-auto inline-block align-middle">
        <h1 class=" text-xl font-medium my-5">Batch List</h1>
            ...
            <table class="">
                <thead>
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3 text-end text-xs font-medium text-gray-500 uppercase dark:text-neutral-500">Start Date</th>
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-neutral-700">
                    ...
                    while ($row = mysqli_fetch_assoc($query)) : ?>
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-neutral-200 text-end" title="<?= $row['title'] ?>"><?= $row['short'] ?></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-neutral-200 text-end"><?= date('d M, Y', strtotime($row['start_date'])) ?></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-neutral-200 text-end">
                                <?= date('g:m A', strtotime($row['start_time']))  ?>~<?= date('g:m A', strtotime($row['end_time'])) ?>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-neutral-200 text-end">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-neutral-200 text-end">
                                <div class="flex flex-col sm:inline-flex sm:flex-row rounded-lg shadow-sm">
                                    ...
                                    <a onclick="return confirm('are u sure u want to delete this row?')" href="./batch-delete.php?row_id=<?= $row['batch_id'] ?>" type="button" class="py-2 px-3 inline-flex justify-center items-center gap-2 -ms-px first:rounded-s-lg first:ms-0 last:rounded-e-lg text-sm font-medium focus:z-10 border border-gray-200 bg-white text-gray-800 shadow-sm hover:bg-gray-50 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-900 dark:border-neutral-700 dark:text-white dark:hover:bg-neutral-800">
